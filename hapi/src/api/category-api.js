...
import { IdSpec, CategorySpec, CategorySpecPlus, CategoryArraySpec, PoiArraySpec, simpleCategorySpec, simpleCategoryArraySpec } from "../models/joi-schemas.js";
...
export const categoryApi = {
  ...
  update: {
    ...
    handler: async function (request, h) {
      try {
        const cat = await db.categoryStore.updateCategory(request.params.id, request.payload);
        if (cat) {
          return h.response(cat).code(202);
        }
    ...
    notes: "Update Category on the DB based on th input parameters",
  },
